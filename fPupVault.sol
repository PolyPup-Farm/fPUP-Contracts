...
contract fPupVault is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 fpupAtLastUserAction; // keeps track of Fpup deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }

    IERC20 public immutable token; // Fpup token
    ...
    /**
     * @notice Constructor
     * @param _token: Fpup token contract
    ...
    /**
     * @notice Deposits funds into the FPUP Vault
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in FPUP)
     */
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.fpupAtLastUserAction = user.shares.mul(balanceOf()).div(
            totalShares
        );
    ...
    /**
     * @notice Reinvests FPUP tokens into MasterChef
     * @dev Only possible when contract not paused.
     */
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Fpup Vault
     */
     ...
     * @return Expected reward to collect in FPUP
     */
    function calculateHarvestFpupRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingFpup(0, address(this));
     ...
     */
    function calculateTotalPendingFpupRewards()
        external
        view
        returns (uint256)
    {
    ...
    /**
     * @notice Withdraws from funds from the Fpup Vault
