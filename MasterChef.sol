...
// fPupToken.
contract fPupToken is BEP20("FTM PUP Token", "fPUP") {
...
// MasterChef is the master of FPUP. He can make Fpup and he is a fair guy.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accFpupPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accFpupPerShare` (and `lastRewardBlock`) gets updated.
    ...
    // Update the given pool's Fpup allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
    ...
    function pendingFpup(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        return user.amount.mul(accFpupPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user
                .amount
                .mul(pool.accFpupPerShare)
                .div(1e18)
                .sub(user.rewardDebt);
            if (pending > 0) {
                safeFpupTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accFpupPerShare).div(1e18);
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accFpupPerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeFpupTransfer(msg.sender, pending);
    ...
    // Safe fpup transfer function, just in case if rounding error causes pool to not have enough FPUPs.
    function safeFpupTransfer(address _to, uint256 _amount) internal {
        uint256 fpupBal = fpup.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > fpupBal) {
            transferSuccess = fpup.transfer(_to, fpupBal);
        } else {
            transferSuccess = fpup.transfer(_to, _amount);
        }
        require(transferSuccess, "safeFpupTransfer: transfer failed");
    }
